...
chain_output_file = "chains/Roman_KL_3x2pt_Ncl%d_Ntomo%d_bary"
bary_pcs_file = "data/Roman_KL_3x2pt_pca.txt"
...
bary_file = os.path.join(outdirname, bary_pcs_file)
...
sample_params += ['bias_%d'%i for i in xrange(Ntomo_lens)]
if NPCs > 0:
    sample_params += ['bary_%d'%i for i in xrange(NPCs)]
